...
class TestBackend(unittest.TestCase):
    ...
    def test_portfolio_flow(self):
        ...
        data = json.loads(resp.data)
        self.assertEqual(data['positions']['005930']['qty'], 10)
        ...
        self.assertEqual(resp.status_code, 200)
        data = json.loads(resp.data)
        self.assertEqual(data['positions']['005930']['qty'], 20)
        self.assertEqual(data['positions']['069500']['qty'], 5)

        # 5. Delete Item (qty=0)
        resp = self.app.post('/api/portfolio', json={'symbol': '005930', 'qty': 0})
        data = json.loads(resp.data)
        self.assertNotIn('005930', data['positions'])
        self.assertIn('069500', data['positions'])
